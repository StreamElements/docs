...
access: moderator
summary: "Add, remove, edit, or alias custom commands from chat."
category: bot-command-management
aliases:
  - "!cmd"
keywords:
  - command command
...
:::note
Listing aliases only works with the original command name, not with one of its aliases.
:::
...
<ChatExample messages={[
  { persona: 'moderator', message: '!command alias add !socials !links' },
  { persona: 'bot', message: '@Moderator, successfully added alias !links to command !socials.' }
]} />

**Example (removing an alias):**
<ChatExample messages={[
  { persona: 'moderator', message: '!command alias remove !socials !links' },
  { persona: 'bot', message: '@Moderator, successfully removed alias !links from command !socials.' }
]} />
...
Configures advanced settings like cooldowns, cost, permission level, response type, trigger name, enabled status, and usage count for a command. See the [Parameters](#parameters-for-options-subcommand) section below for details on the available options.
...
:::note
Replace `option1`, `value1`, etc., with the specific parameter flags and values listed in the Parameters section below. You can specify multiple options.
:::

**Example (setting multiple options):**
<ChatExample messages={[
  { persona: 'moderator', message: '!command options !socials -cd 10 -usercd 30 -cost 50 -level 500' },
  { persona: 'bot', message: '@Moderator, successfully updated command !socials.' }
]} />
...
*   **How do I add or remove a command alias via chat?**
    Use the `alias` subcommand with `add` or `remove`: `!command alias add !<command_name> !<alias_name>` or `!command alias remove !<command_name> !<alias_name>`.
