...
| `-cost`          | Sets the **loyalty points cost** charged each time the command is used. If a user doesn't have enough points, the command silently does nothing (no error message is sent). Requires the StreamElements Loyalty System (enabled in the dashboard under **Loyalty** → **Loyalty Settings**). | `<points>` (e.g., `50`)                                                      | `-cost 50`                                |
