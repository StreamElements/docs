...
| `$(channel.provider)` | Channel's streaming platform as a lowercase slug: `twitch`, `youtube`, `trovo`, or `kick` | |
...
- This cross-channel form works on Twitch (and partially on Trovo). It is not implemented on YouTube, where it falls back to your own channel's data.
