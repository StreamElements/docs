...
Replace `<query>` with the text you want the AI to respond to.
...
In these examples, `${1}` inserts only the first word the viewer types after the command, while `${1:}` inserts everything from the first word onward — see [$(args)](/chatbot/variables/args/) for more on argument variables.
