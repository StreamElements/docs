        ...
        async for raw in websocket:
            ...
            if msg_type == "welcome":
                ...
                if topic == "channel.activities":
                    ...
                    print(f"Session reset: {message['data']}")
...
# Unsubscribe from a specific room, or from all rooms for the topic when room is omitted
...
No additional dependencies are required — `System.Net.WebSockets` and `System.Text.Json` are part of the .NET base class library.
...
class StreamElementsClient
{
    ...
    // Unsubscribe from a specific room, or from all rooms for the topic when room is null
...
// Unsubscribe from a specific room
func unsubscribe(conn *websocket.Conn, topic, room string) error {
...
// Unsubscribe from all rooms for a topic
func unsubscribeAll(conn *websocket.Conn, topic string) error {
	return conn.WriteJSON(map[string]interface{}{
		"type":  "unsubscribe",
		"nonce": uuid.New().String(),
		"data": map[string]interface{}{
