...
The session `data` object holds per-feature totals, goals, and latest-event details for your channel — the same values visible in the [Widget Data dashboard](/overlays/widget-data). Custom widgets receive it through the [onWidgetLoad and onSessionUpdate events](/overlays/events):
