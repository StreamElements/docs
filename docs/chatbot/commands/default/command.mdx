...
Shows the aliases associated with a specific command, or adds a new alias to it.
...
*   **How do I add an alias to a command via chat?**
    Use the `alias` subcommand with `add`: `!command alias add !<command_name> !<alias_name>`.
